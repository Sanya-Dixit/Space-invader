...
# Power-ups
power_up_types = ['health', 'speed']
power_up_imgs = {
    'health': pygame.image.load("images/health_powerup.png"),
    'speed': pygame.image.load("images/speed_powerup.png")
}
power_up_x = []
power_up_y = []
power_up_x_change = []
power_up_y_change = 0.5
...
def power_up(x, y, type):
    screen.blit(power_up_imgs[type], (x, y))

# Function to spawn power-ups
def spawn_power_up():
    if random.random() < 0.02:  # 2% chance to spawn a power-up
        power_type = random.choice(power_up_types)
        x = random.randint(0, 735)
        y = 50  # Spawn at the top of the screen
        power_ups.append({'x': x, 'y': y, 'type': power_type})
...
def collect_power_up():
    ...
    for power_up in power_ups[:]:
        if is_collision(playerX, playerY, power_up['x'], power_up['y']):
            power_ups.remove(power_up)
            if power_up['type'] == 'health':
                player_health += 1  # Increase health
            elif power_up['type'] == 'speed':
                playerX_change *= 1.5  # Increase player speed
...
while running:
    ...
    for power_up in power_ups[:]:
        power_up['y'] += power_up_y_change
        if power_up['y'] > 600:
            power_ups.remove(power_up)
        power_up(power_up['x'], power_up['y'], power_up['type'])
